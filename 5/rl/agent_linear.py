...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if p == 1:
        ...
        action_index, object_index = index2tuple(np.argmax((theta @ state_vector), axis=None))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if terminal:
        ...
        maxQ = (theta @ next_state_vector).max()
        theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + ALPHA*(reward + GAMMA*maxQ - q_value)*current_state_vector
