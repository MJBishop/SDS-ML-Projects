...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for row in feature_matrix:
        loss_sum += hinge_loss_single(row, labels[k], theta, theta_0) 
        k += 1
    return loss_sum / k
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    #Your code here
    current_theta = np.zeros([feature_matrix.shape[1],]) #feature_matrix.shape[1] => number of columns (features)
    current_theta_0 = 0
    for t in range(T):
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    classification_array = np.zeros([feature_matrix.shape[0],])
    i = 0
    for row in feature_matrix:
        if np.dot(theta, row) + theta_0 > 0:
...
def classifier_accuracy(
    ...
    return (
        accuracy(train_classification, train_labels), 
        accuracy(val_classification, val_labels))
...
def bag_of_words(texts):
    ...
    # Your code here
    try:
        with open(str('stopwords.txt')) as fp:
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                ...
                feature_matrix[i, dictionary[word]] += 1 # '+=' uses count instead of binary indicator
